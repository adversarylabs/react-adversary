...
test("accepts reactive, observed, static and unrelated-ref layouts", () => {
 for (const text of [fixture("[markup]"), fixture("[]", "new MutationObserver(update).observe(box.current, {childList:true});"), fixture("[]", "", "'static'"), fixture("[]", "", "markup", "other"), fixture().replace('from "react"', 'from "custom-hooks"')])
